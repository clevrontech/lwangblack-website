// LWANG BLACK ADMIN — Core Logic (Firebase Realtime Backend)
...
const API='/api';
let currentUser=null,authToken=null,refreshToken=null,allOrders=[],orderSort={col:'date',dir:-1},orderTab='all',ordPage=1,bulkSelected=new Set();
let _pollInterval=null,_lastOrderCount=0,_lastPendingCount=0,_pollActive=false;
...
async function doLogin(){
  ...
  try{
    ...
    authToken=d.token;refreshToken=d.refreshToken;currentUser=d.user;
    localStorage.setItem('lb_adm_token',authToken);
    localStorage.setItem('lb_adm_refresh',refreshToken||'');
...
function doLogout(){
  stopPolling();
  if(authToken&&authToken!=='local'){fetch(`${API}/auth/logout`,{method:'POST',headers:{Authorization:`Bearer ${authToken}`}}).catch(()=>{});}
  localStorage.removeItem('lb_adm_token');localStorage.removeItem('lb_adm_refresh');localStorage.removeItem('lb_adm_user');
  currentUser=null;authToken=null;refreshToken=null;
  document.getElementById('loginWrap').style.display='flex';document.getElementById('dashboard').style.display='none';
}
...
function boot(){
  ...
  // Mobile mirrors
  const mob=document.getElementById('mobAvatar');
  const mobN=document.getElementById('mobName');
  const mobR=document.getElementById('mobRole');
  if(mob)mob.textContent=(currentUser.name||'A')[0].toUpperCase();
  if(mobN)mobN.textContent=currentUser.name||currentUser.username;
  if(mobR)mobR.textContent=isOwner?'👑 Owner':`📍 ${CNAMES[currentUser.country]||currentUser.country||'Manager'}`;
  // Expose globally for admin-logic3.js
...
// ── REAL-TIME POLLING (replaces WebSocket — Vercel serverless compatible) ──
function setWsStatus(live){
  const dot=document.getElementById('wsLiveDot');
  const lbl=document.getElementById('wsLiveLabel');
  const wrap=document.getElementById('wsLiveStatus');
  if(!dot)return;
  if(live){
    dot.style.background='var(--accent2)';dot.style.animation='livePulse 2s infinite';
    if(lbl)lbl.textContent='Live';
    if(wrap)wrap.style.borderColor='rgba(76,175,135,.35)';
  }else{
    dot.style.background='rgba(255,107,107,.7)';dot.style.animation='none';
    if(lbl)lbl.textContent='Offline';
    if(wrap)wrap.style.borderColor='rgba(255,107,107,.25)';
  }
}
...
async function tryRefreshToken(){
  ...
  try{
    ...
    const d=await r.json();
    authToken=d.token;window.authToken=authToken;
    localStorage.setItem('lb_adm_token',authToken);
    return true;
  }catch{return false;}
}
...
function startPolling(){
  ...
  _pollInterval=setInterval(async()=>{
    if(!authToken||!_pollActive)return;
    try{
      const r=await apiFetch(`${API}/analytics/realtime`);
      if(!r.ok){if(r.status===401){setWsStatus(false);}return;}
      const d=await r.json();
      // Detect new orders
      if(_lastOrderCount>0&&d.totalOrders>_lastOrderCount){
        const diff=d.totalOrders-_lastOrderCount;
        showToast(`🔔 ${diff} new order${diff>1?'s':''} received!`);
        await loadOrders();
        const pg=document.querySelector('.page.active');
        if(pg?.id==='page-home')renderHome();
        if(pg?.id==='page-orders')renderOrders();
      }
      // Detect new pending orders
      if(_lastPendingCount>=0&&d.pendingOrders>_lastPendingCount){
        const el=document.getElementById('ordBadge');
        if(el)el.textContent=d.pendingOrders||0;
      }
      _lastOrderCount=d.totalOrders||0;
      _lastPendingCount=d.pendingOrders||0;
      setWsStatus(true);
    }catch(e){
      setWsStatus(false);
    }
...
// Keep connectWebSocket name for compatibility with admin-logic3.js
function connectWebSocket(){startPolling();}
function showToast(msg){
  let t=document.getElementById('adminToast');
  if(!t){t=document.createElement('div');t.id='adminToast';t.style.cssText='position:fixed;bottom:2rem;right:2rem;background:rgba(20,20,20,0.95);color:#fff;padding:1rem 1.5rem;border-radius:12px;font-size:0.8rem;z-index:10000;transform:translateY(100px);opacity:0;transition:all 0.3s;border:1px solid rgba(255,255,255,0.1);backdrop-filter:blur(20px);max-width:350px;';document.body.appendChild(t);}
...
function getDemoOrders(){
  return[
    {id:'LB-2450',date:new Date(Date.now()-86400000).toISOString(),status:'paid',country:'AU',currency:'AUD',symbol:'A$',items:[{name:'Lwang Black 250g',qty:2,price:27,image:'https://cdn2.blanxer.com/uploads/68b26f1169953999df49c53a/product_image-dsc07401-8095.webp'}],subtotal:54,shipping:14.99,total:68.99,carrier:'Australia Post',tracking:'',customer:{fname:'Emma',lname:'Wilson',email:'emma@email.au',phone:'+61412345678',address:'12 George St Sydney NSW 2000'},payment:{method:'paypal',status:'paid',ref:'PP-5X9284K'}},
    {id:'LB-2449',date:new Date(Date.now()-172800000).toISOString(),status:'shipped',country:'AU',currency:'AUD',symbol:'A$',items:[{name:'Lwang Black 500g',qty:1,price:37},{name:'French Press',qty:1,price:34.99}],subtotal:71.99,shipping:14.99,total:86.98,carrier:'Australia Post',tracking:'AUP123456',customer:{fname:'Liam',lname:'Chen',email:'liam@email.au',phone:'+61487654321',address:'45 Collins St Melbourne VIC 3000'},payment:{method:'afterpay',status:'paid',ref:'AP-8876543'}},
    {id:'LB-2448',date:new Date(Date.now()-259200000).toISOString(),status:'delivered',country:'NP',currency:'NPR',symbol:'Rs',items:[{name:'Lwang Black 500g',qty:2,price:2599}],subtotal:5198,shipping:0,total:5198,carrier:'Pathao',tracking:'',customer:{fname:'Aarav',lname:'Shrestha',email:'aarav@email.np',phone:'+977984123456',address:'Durbarmarg, Kathmandu'},payment:{method:'esewa',status:'paid',ref:'ESW-2448'}},
    {id:'LB-2447',date:new Date(Date.now()-345600000).toISOString(),status:'pending',country:'US',currency:'USD',symbol:'$',items:[{name:'Pot & Press Gift Set',qty:1,price:69.99}],subtotal:69.99,shipping:15,total:84.99,carrier:'Australia Post',tracking:'',customer:{fname:'Jake',lname:'Miller',email:'jake@email.us',phone:'+14155551234',address:'580 California St San Francisco CA'},payment:{method:'apple_pay',status:'pending',ref:null}},
    {id:'LB-2446',date:new Date(Date.now()-432000000).toISOString(),status:'paid',country:'GB',currency:'GBP',symbol:'£',items:[{name:'Lwang Black 250g',qty:1,price:11.99},{name:'T-Shirt',qty:1,price:15.99}],subtotal:27.98,shipping:11.99,total:39.97,carrier:'Australia Post',tracking:'',customer:{fname:'Oliver',lname:'Smith',email:'oliver@email.uk',phone:'+447911123456',address:'10 Finsbury Sq London EC2A'},payment:{method:'google_pay',status:'paid',ref:'GP-3ghi789'}},
    {id:'LB-2445',date:new Date(Date.now()-518400000).toISOString(),status:'delivered',country:'CA',currency:'CAD',symbol:'C$',items:[{name:'LB Drip & Sip Set',qty:1,price:29.99}],subtotal:29.99,shipping:15.99,total:45.98,carrier:'Chit Chats',tracking:'CHT789012',customer:{fname:'Sophie',lname:'Brown',email:'sophie@email.ca',phone:'+14165551234',address:'100 King St W Toronto ON'},payment:{method:'stripe',status:'paid',ref:'pi_3jkl012'}},
    {id:'LB-2444',date:new Date(Date.now()-604800000).toISOString(),status:'shipped',country:'NZ',currency:'NZD',symbol:'NZ$',items:[{name:'250g + 500g Lwang Black Bundle',qty:1,price:49.99}],subtotal:49.99,shipping:12.99,total:62.98,carrier:'NZ Post',tracking:'NZP345678',customer:{fname:'Ella',lname:'Taylor',email:'ella@email.nz',phone:'+6421345678',address:'151 Queen St Auckland'},payment:{method:'card',status:'paid',ref:'MC-3mno345'}},
    {id:'LB-2443',date:new Date(Date.now()-691200000).toISOString(),status:'cancelled',country:'JP',currency:'JPY',symbol:'¥',items:[{name:'Lwang Black 250g',qty:1,price:2299}],subtotal:2299,shipping:0,total:2299,carrier:'Japan Post',tracking:'',customer:{fname:'Yuki',lname:'Tanaka',email:'yuki@email.jp',phone:'+81312345678',address:'2-1-1 Nihonbashi Tokyo'},payment:{method:'paypal',status:'failed',ref:null}},
    {id:'LB-2442',date:new Date(Date.now()-777600000).toISOString(),status:'delivered',country:'NP',currency:'NPR',symbol:'Rs',items:[{name:'Lwang Black 250g',qty:1,price:1599}],subtotal:1599,shipping:0,total:1599,carrier:'Pathao',tracking:'COD-KTM001',customer:{fname:'Sita',lname:'Gurung',email:'sita@email.np',phone:'+977985000001',address:'Thamel, Kathmandu'},payment:{method:'cod',status:'paid',ref:'COD-2442'}},
  ];
}
...
async function loadOrders(){
  try{
    ...
    if(r.ok){
      ...
      _lastPendingCount=allOrders.filter(o=>o.status==='pending' || o.order_status==='processing').length;
      const el=document.getElementById('ordBadge');
      if(el)el.textContent=_lastPendingCount||0;
      return;
    }
  }catch(err){console.warn('[loadOrders] API failed, using local fallback:',err.message);}
  ...
  _lastOrderCount=allOrders.length;
  _lastPendingCount=allOrders.filter(o=>o.status==='pending').length;
  const el=document.getElementById('ordBadge');
  if(el)el.textContent=_lastPendingCount||0;
}
...
function renderHome(){
  ...
  const acts=[
    ...
    {icon:'🚚',text:`Order <b>${allOrders[1]?.id||'LB-2449'}</b> shipped via Australia Post`,time:'1h ago'},
...
function renderOrders(){
  ...
  if(pf!=='all')orders=orders.filter(o=>(o.payment?.method||o.paymentMethod||'')===pf);
  ...
  document.getElementById('ordersBody').innerHTML=paged.length?paged.map(o=>`<tr>
    <td data-label="✓"><input type="checkbox" class="cb bulk-cb" value="${o.id}" onchange="toggleBulk('${o.id}',this)"></td>
    <td data-label="ID"><a class="tbl-link mono" onclick="openOrderModal('${o.id}')">${o.id}</a></td>
    <td data-label="Customer">${o.customer?.fname||'—'} ${o.customer?.lname||''}<br><span style="font-size:.68rem;color:var(--muted)">${o.customer?.email||''}</span></td>
    <td data-label="Country">${FLAGS[o.country]||''} ${CNAMES[o.country]||o.country}</td>
    <td data-label="Items">${o.items?.length||0} item(s)</td>
    <td data-label="Total" style="font-weight:600">${o.symbol}${(o.total||0).toLocaleString()}</td>
    <td data-label="Payment">${payMethodPill(o.payment?.method||o.paymentMethod||'—')}</td>
    <td data-label="Date" style="color:var(--muted);font-size:.72rem">${fmtDate(o.date)}</td>
    <td data-label="Status">${pill(o.status)}</td>
    <td data-label="Actions"><div style="display:flex;gap:.4rem;justify-content:flex-end">
...
async function updateStatus(id,status,silent=false){
  try{
    ...
    console.warn('Update status API failed, trying legacy:', err.message);
    try{
    ...
    <div class="card" style="margin:0"><div class="card-title" style="margin-bottom:.75rem">Shipping & Tracking</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem">
        <div class="form-row">
          ...
          <select class="form-input" id="providerSelect_${o.id}">
            <option value="auspost" ${o.shipping_provider === 'auspost' ? 'selected' : ''}>Australia Post</option>
            <option value="chitchats" ${o.shipping_provider === 'chitchats' ? 'selected' : ''}>Chit Chats</option>
            <option value="nzpost" ${o.shipping_provider === 'nzpost' ? 'selected' : ''}>NZ Post</option>
            <option value="japanpost" ${o.shipping_provider === 'japanpost' ? 'selected' : ''}>Japan Post</option>
            <option value="pathao" ${o.shipping_provider === 'pathao' ? 'selected' : ''}>Pathao</option>
          </select>
        </div>
        <div class="form-row">
          <label class="form-label">Tracking ID</label>
          <input class="form-input" id="trackingInput_${o.id}" value="${o.tracking_id || ''}" placeholder="Enter tracking number..."/>
        </div>
      </div>
      <div style="display:flex;gap:.75rem;align-items:center;flex-wrap:wrap">
        <select class="form-input" style="max-width:160px" id="statusSelect_${o.id}">
          ${['pending','paid','shipped','delivered','cancelled'].map(s=>`<option value="${s}"${(o.order_status || o.status)===s?' selected':''}>${s}</option>`).join('')}
        </select>
        <button class="btn btn-primary" onclick="saveOrderTracking('${o.id}')">Save Logistics</button>
        <button class="btn btn-ghost" onclick="printInvoice('${o.id}')">🖨 Invoice</button>
        ${(o.order_status || o.status)!=='cancelled'?`<button class="btn btn-danger" onclick="if(confirm('Cancel order?')){updateStatus('${o.id}','cancelled');closeModal('modalOrder');}">Cancel</button>`:''}
      </div>
    </div>`;
...
async function saveOrderTracking(id){
  ...
  const tracking_id = document.getElementById('trackingInput_'+id)?.value || '';
  const shipping_provider = document.getElementById('providerSelect_'+id)?.value || o.shipping_provider;
  const status = document.getElementById('statusSelect_'+id)?.value || o.order_status || o.status;

  try {
    const res = await apiFetch(`${API}/admin/orders`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id, status, shipping_provider, tracking_id })
    });
    
    if (res.ok) {
      o.tracking_id = tracking_id;
      o.shipping_provider = shipping_provider;
      o.status = status;
      o.order_status = status;
      saveLocalOrders();
    ...
    console.error('Save logistics error:', err);
    // Local fallback
    o.tracking_id = tracking_id;
    o.shipping_provider = shipping_provider;
    o.status = status;
