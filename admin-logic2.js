...
function renderProducts(){
  ...
  document.getElementById('productsBody').innerHTML=filtered.map(p=>{
    ...
    return`<tr>
      <td data-label="Image"><img class="prod-thumb" src="${p.image}" loading="lazy" onerror="this.src='images/logo-hero.png'"/></td>
      <td data-label="Name"><a class="tbl-link" onclick="openProductModal('${p.id}')">${p.name}</a></td>
      <td data-label="Category"><span class="pill pill-blue">${p.category}</span></td>
      <td data-label="NPR">${p.prices?.NP?.display||'—'}</td><td data-label="AUD">${p.prices?.AU?.display||'—'}</td><td data-label="USD">${p.prices?.US?.display||'—'}</td>
      <td data-label="Stock"><span class="pill ${sc}">${sl}</span></td>
      <td data-label="Regions" style="font-size:.72rem;color:var(--muted)">${p.allowed_regions==='ALL'?'🌍 Global':(p.allowed_regions||[]).join(', ')}</td>
      <td data-label="Status"><span class="pill ${s>0?'pill-green':'pill-red'}">${s>0?'Active':'Inactive'}</span></td>
...
function renderCustomers(){
  ...
  document.getElementById('customersBody').innerHTML=custs.map(c=>`<tr>
    <td data-label="Customer"><div style="display:flex;align-items:center;gap:.75rem"><div class="sb-avatar" style="width:32px;height:32px;font-size:.75rem">${(c.fname||'?')[0]}</div><div style="font-weight:600">${c.fname||'—'} ${c.lname||''}</div></div></td>
    <td data-label="Email" style="color:var(--muted)">${c.email||'—'}</td>
    <td data-label="Country">${FLAGS[c.country]||''} ${CNAMES[c.country]||c.country}</td>
    <td data-label="Orders">${c.orders.length}</td>
    <td data-label="Total ($)" style="font-weight:600;color:var(--accent2)">$${fmt(c.totalUSD)}</td>
    <td data-label="Last Order" style="color:var(--muted);font-size:.72rem">${fmtDate(c.lastOrder)}</td>
    <td data-label="Actions"><button class="btn btn-ghost btn-sm" onclick="openCustomerModal('${c.email||c.fname}')">View</button></td>
...
function openCustomerModal(email){
  ...
  document.getElementById('modalCustomerBody').innerHTML=`
    ...
    <div class="card" style="margin:0"><div class="card-title" style="margin-bottom:.75rem">Order History</div>
      ...
      <tbody>${c.orders.map(o=>`<tr><td data-label="Order"><a class="tbl-link" onclick="closeModal('modalCustomer');openOrderModal('${o.id}')">${o.id}</a></td><td data-label="Date" style="color:var(--muted)">${fmtDate(o.date)}</td><td data-label="Amount">${o.symbol}${(o.total||0).toLocaleString()}</td><td data-label="Status">${pill(o.status)}</td></tr>`).join('')}</tbody></table></div>`;
...
function renderFinance(){
  ...
  const byMethod={};
  paid.forEach(o=>{const m=o.payment?.method||o.paymentMethod||'other';byMethod[m]=(byMethod[m]||0)+1;});
  const pending=allOrders.filter(o=>o.payment?.status==='pending').length;
  const codPending=allOrders.filter(o=>(o.payment?.method||o.paymentMethod)==='cod'&&o.payment?.status!=='paid').length;
  document.getElementById('financeStats').innerHTML=`
    ...
    <div class="stat-card"><div class="stat-label">PayPal</div><div class="stat-val">${byMethod.paypal||0}</div><div class="stat-change up">transactions</div></div>
    <div class="stat-card"><div class="stat-label">Stripe/Card</div><div class="stat-val">${(byMethod.stripe||0)+(byMethod.card||0)+(byMethod.apple_pay||0)+(byMethod.google_pay||0)}</div><div class="stat-change up">transactions</div></div>
    <div class="stat-card"><div class="stat-label">eSewa</div><div class="stat-val">${byMethod.esewa||0}</div><div class="stat-change up">Nepal</div></div>
    <div class="stat-card"><div class="stat-label">Afterpay</div><div class="stat-val">${byMethod.afterpay||0}</div><div class="stat-change up">BNPL</div></div>
    <div class="stat-card"><div class="stat-label">COD</div><div class="stat-val">${byMethod.cod||0}</div><div class="stat-change ${codPending?'down':'up'}">${codPending?codPending+' uncollected':'Nepal'}</div></div>
    <div class="stat-card"><div class="stat-label">Pending</div><div class="stat-val">${pending}</div><div class="stat-change down">awaiting</div></div>`;
  ...
  const pmLabels={paypal:'PayPal',stripe:'Stripe',apple_pay:'Apple Pay',google_pay:'Google Pay',afterpay:'Afterpay',card:'Mastercard/Debit',esewa:'eSewa',cod:'COD'};
  const pmColors=['rgba(0,112,210,.7)','rgba(99,91,255,.7)','rgba(200,200,200,.7)','rgba(66,133,244,.7)','rgba(183,209,131,.7)','rgba(235,120,40,.7)','rgba(76,175,135,.7)','rgba(255,193,7,.7)'];
  const pmKeys=Object.keys(byMethod);
  if(window._cPM)window._cPM.destroy();
  window._cPM=new Chart(document.getElementById('chartPayMethods'),{type:'doughnut',data:{labels:pmKeys.map(k=>pmLabels[k]||k),datasets:[{data:pmKeys.map(k=>byMethod[k]),backgroundColor:pmKeys.map((_,i)=>pmColors[i%pmColors.length]),borderColor:'transparent'}]},options:{responsive:true,cutout:'65%',plugins:{legend:{position:'bottom',labels:{color:'rgba(232,233,234,.5)',font:{size:11},boxWidth:10}}}}});
  document.getElementById('financeBody').innerHTML=[...allOrders].sort((a,b)=>new Date(b.date)-new Date(a.date)).map(o=>`<tr>
    ...
    <td>${payMethodPill(o.payment?.method||o.paymentMethod||'—')}</td>
...
const payMethods={NP:'eSewa + COD',AU:'PayPal + Stripe + Apple Pay + Afterpay + Google Pay',US:'PayPal + Stripe + Apple Pay + Afterpay + Google Pay',GB:'PayPal + Stripe + Apple Pay + Afterpay + Google Pay',CA:'PayPal + Stripe + Apple Pay + Google Pay',NZ:'PayPal + Stripe + Apple Pay + Afterpay + Google Pay',JP:'PayPal + Stripe + Google Pay'};
...
function saveSettings(){
  const c={name:document.getElementById('cfg-name')?.value,email:document.getElementById('cfg-email')?.value,whatsapp:document.getElementById('cfg-whatsapp')?.value,stripe_pub:document.getElementById('cfg-stripe-pub')?.value,paypal_id:document.getElementById('cfg-paypal-id')?.value,esewa_id:document.getElementById('cfg-esewa-id')?.value};
...
function pill(status,type){
  const map={paid:'pill-green',delivered:'pill-green',active:'pill-green',shipped:'pill-blue',stripe:'pill-blue',esewa:'pill-green',paypal:'pill-blue',cod:'pill-yellow',cod_pending:'pill-yellow',apple_pay:'pill-grey',google_pay:'pill-blue',afterpay:'pill-green',card:'pill-blue',pending:'pill-yellow',processing:'pill-yellow',failed:'pill-red',cancelled:'pill-red',info:'pill-blue'};
  return`<span class="pill ${type?map[type]:''}${!type?(map[status]||'pill-grey'):''}">${status}</span>`;
}
function payMethodPill(method){
  const labels={paypal:'🅿️ PayPal',stripe:'💳 Stripe',apple_pay:'🍎 Apple Pay',google_pay:'🔵 Google Pay',afterpay:'🟩 Afterpay',card:'💳 MC/Debit',esewa:'🟢 eSewa',cod:'💵 COD'};
  const colors={paypal:'pill-blue',stripe:'pill-purple',apple_pay:'pill-grey',google_pay:'pill-blue',afterpay:'pill-green',card:'pill-blue',esewa:'pill-green',cod:'pill-yellow'};
  const label=labels[method]||method;
  const cls=colors[method]||'pill-grey';
