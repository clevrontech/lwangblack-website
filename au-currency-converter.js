  ...
  try {
    ...
    document.addEventListener('lb:regionChanged', (e) => {
      ...
      if (e.detail.code !== 'AU') {
        ...
        // If switched to AU (or on AU), attempt auto-set
        autoSetCurrencyFromGeo();
      }
    });
...
function autoSetCurrencyFromGeo() {
  if (window.AUCurrencyState.autoSetDone) return;
  if (!window.GeoRouter) return;
  
  const setupAutoSet = (raw) => {
    if (!raw) return;
    // Try to find raw country code in countriesData
    if (countriesData.some(x => x.code === raw)) {
      const sel = document.getElementById('au-currency-select');
      if (sel && sel.value === 'DEFAULT') {
        sel.value = raw;
        sel.dispatchEvent(new Event('change'));
        window.AUCurrencyState.autoSetDone = true;
      }
    }
  };

  if (window.GeoRouter.rawCountryCode) {
    setupAutoSet(window.GeoRouter.rawCountryCode);
  } else {
    // If not detected yet (e.g. they had a stored region), detect it now in the background
    window.GeoRouter.detect().then(() => {
      setupAutoSet(window.GeoRouter.rawCountryCode);
    }).catch(() => {});
  }
}
...
function buildConverterUI() {
  ...
  // If already AU, try to auto set immediately in case GeoRouter finished early
  if (currentRegion === 'AU') {
    autoSetCurrencyFromGeo();
  }
